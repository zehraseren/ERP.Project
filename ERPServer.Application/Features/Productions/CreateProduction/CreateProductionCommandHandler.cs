    ...
    public async Task<Result<string>> Handle(CreateProductionCommand request, CancellationToken cancellationToken)
    {
        ...
        if (recipe is not null && recipe.Details is not null)
        {
            ...
            foreach (var item in details)
            {
                ...
                if (item.Quantity > stock) return Result<string>.Failure($"{item.Product!.Name} ürününden üretim için yeterli miktar bulunmamaktadır. Eksik Miktar: {item.Quantity - stock}");
